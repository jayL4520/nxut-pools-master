    ...
    <div class="poolPage_title titls pad t-center flex-box center white bold" v-if="$route.name=='pool'">
      ...
      <div class="flex-box center marLeft" v-show="$route.name!=='index'">    <el-button size="mini" type="warning" @click="$router.push('/'+$route.params.wxid)">返回</el-button> </div>
        ...
        <div  class="coll_100 marBtm" >
          ...
          <div class=" center flex-box center column mar coll_100" >
            ...
            <!-- <svg  :class="{
              ...
              <div class=" xs_marLeft  bold">矿池{{(!$route.params.id?'列表':'详情')+(loading?' 加载中...':'')}}</div>
              </div>
            ...
            <div class="sqbox center flex-box coll_100 " v-for="(item,i) in msgData" >
              ...
              <div class="titl act padTop padBtm coll_20 cusor" :class="{isCli:i=='name'}" @click="$router.push('/'+$route.params.wxid+'/'+item.id)"
               v-for="(el,i) in  msgData[0]">    {{['id','type','time'].indexOf(i)==-1&&Number(item[i])>=0?Number(item[i]).toFixed(2):
...
export default {
  ...
  methods:{
   async getData(i){
    this.state = i
    this.loading = true
    let url = `/index/index/kuang_list?state=${i}&wxid=${this.$route.params.wxid}`

    if(this.$route.params.id){
      debugger
      url = `/index/index/hashlog_list?kid=${this.$route.params.id}`
    }
