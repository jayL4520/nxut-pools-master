    ...
    <!-- <div class="poolPage_title pad t-center blue bold"></div> -->
      <div class="row flex-box item-start">
        <div :span="8" class="coll_20 padLeft" style="min-width: 140px;">
          ...
          <!-- <div class="xp-box flex-box cusor" @click="getCount(1)">
            <div class="names" :class="{'xp-name':state==0}">详细流水</div>
            <div class="xp-label act">></div>
          </div>

          <div class="xp-box flex-box cusor" @click="getCount(0)" >
            <div class="names" :class="{'xp-name':state==1}">矿池收益</div>
            <div class="xp-label act">></div>
          </div> -->
        </div>
        <!-- <div :lg="16" class="coll_80">
          
          <div class=" center flex-box column mar coll_100">
            <div class="flex-box">
            ...
            <svg  :class="{
              ...
              <div class="marBtm">{{(state?'矿池收益':'详细流水')+(loading?' 加载中...':'')}}</div>
              </div>
            <div class="sqbox marTop center flex-box coll_100">
              <div class="titl act marBtm coll_20"  v-for="(item,i) in msgData[0]" >{{i}}</div>
            </div>
            <div class="tables coll_100">
            <div class="sqbox center flex-box coll_100 " v-for="(item,i) in msgData" >
            
              <div class="titl act marBtm coll_20"  v-for="(el,i) in  msgData[0]"> {{Number(item[i])>=0?Number(item[i]).toFixed(3):item[i]}}</div>
            </div>
          </div>
...
export default {
  ...
  mounted(){
    ...
    this.getData()
    console.log("this.$route.params.wxid",this.$route.params)
  },
  methods:{

    async getData(wxid){
      // this.loading = true
     let res =  await this.$axios.get(`/index/index/get_count?page=${this.page}&wxid=${this.$route.params.wxid}`)
     if(res){
      this.info =  res
     }
